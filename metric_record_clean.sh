...
# 接收参数，获取Prometheus的访问地址，如"172.30.233.80:9090"
PROMETHEUS_URL=
if [ "$#" -ne 1 ];then
	echo "参数：Prometheus访问地址"
	exit 1
else
	PROMETHEUS_URL="$1"
fi

# 下面这条for语句里列出的是Prometheus所有的监控指标
...
do
    # 需要保留监控数据的指标，都写在下面这条for语句里
    for j in {"container_cpu_usage_seconds_total","container_memory_usage_bytes","container_network_receive_bytes_total","container_network_transmit_bytes_total"}
    do
	if [ "$i" = "$j" ];then
		# 注意：这里的2非常重要，用来跳过外面的删除语句
        	continue 2
	fi
    done

    # 删除该监控指标的数据记录
    curl -XPOST -g "$PROMETHEUS_URL/api/v1/admin/tsdb/delete_series?match[]=$i" \
        || echo "fail to remove record of $i"
done

# 清理tombstone（删除数据记录时会产生tombstone）
curl -XPOST "$PROMETHEUS_URL"/api/v1/admin/tsdb/clean_tombstones \
